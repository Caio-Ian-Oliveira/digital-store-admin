...
interface ProductImageUploadProps {
	...
	setExistingImagesView: (
		images: { id?: string; url: string; type: string; file?: File }[],
	) => void;
	selectedFiles: File[];
...
export function ProductImageUpload({
	...
	const handleFileSelect = (files: FileList | null) => {
		...
		setValue(
			"images",
			allImagesForValidation.map((img) => ({
				type: img.type,
				content: img.url,
			})),
			{ shouldValidate: true },
	...
	const removeSelectedFile = (index: number) => {
		...
		const updatedExistingView = existingImagesView.filter(
			(_, i: number) => i !== index,
		);
		...
		setValue(
			"images",
			updatedExistingView.length > 0
				? updatedExistingView.map((img: any) => ({
						type: img.type,
						content: img.url,
					}))
				: [],
	...
	const moveImage = (index: number, direction: "up" | "down") => {
		...
		if (targetIndex >= 0 && targetIndex < newImgs.length) {
			...
			setValue(
				"images",
				newImgs.map((img) => ({ type: img.type, content: img.url })),
			);
		}
	};
		...
		<section className="space-y-5">
			...
			>
				...
				<p className="text-xs text-gray-400 font-medium px-4">
					Arraste suas imagens ou clique para buscar arquivos locais
				</p>
				<input
					id="file-upload"
					type="file"
			...
			<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar p-1">
				{existingImagesView.map((img, index) => (
					<div
						key={`${img.url}-${index}`}
						className="flex flex-col p-3 bg-white rounded-xl border border-gray-100 shadow-sm group hover:border-primary/30 transition-all"
					>
