			...
			<section className="space-y-6">
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-5">
					...
					<div className="space-y-2">
						...
						/>
						{errors.priceWithDiscount && (
							<span className="text-xs text-error font-medium">
								{String(errors.priceWithDiscount.message)}
							</span>
						)}
					</div>
				</div>
				...
				<div className="flex items-center justify-between p-4 rounded-2xl bg-primary/5 border border-primary/10">
					...
					<span className="text-xl font-black text-primary">
						{calculatedDiscountPrice !== null
							? `R$ ${calculatedDiscountPrice.toFixed(2)}`
							: "R$ 0.00"}
					</span>
				</div>
				...
				<div className="flex flex-col gap-3 pt-2">
					<label className="flex items-center gap-3 cursor-pointer group">
						<input
							type="checkbox"
							{...register("enabled")}
							className="w-5 h-5 rounded-md border-gray-300 text-primary accent-primary"
						/>
			...
			<section className="space-y-6">
				...
				<div className="space-y-4">
					{optionFields.map((field, index) => (
						<div
							key={field.id}
							className="p-5 border border-gray-100 rounded-xl bg-white shadow-sm relative group hover:shadow-md transition-all border-l-4 border-l-primary/30"
						>
							<button
								type="button"
							...
							<div className="grid grid-cols-1 gap-4 mb-4">
								<div className="space-y-1">
									...
									/>
									{(errors.options as any)?.[index]?.title && (
										<span className="text-[10px] text-error font-medium">
											{String((errors.options as any)[index].title.message)}
										</span>
									)}
								</div>
								<div className="space-y-1">
							...
							<div className="space-y-1">
								...
								/>
								{(errors.options as any)?.[index]?.values && (
									<span className="text-[10px] text-error font-medium">
										{String((errors.options as any)[index].values.message)}
									</span>
								)}
							</div>
						</div>
			...
			<section className="space-y-5">
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-[250px] overflow-y-auto p-5 bg-gray-50/50 rounded-2xl border border-gray-100 custom-scrollbar">
					{categories.map((cat) => (
						<label
							key={cat.id}
							className="flex items-center gap-3 p-3 bg-white rounded-xl cursor-pointer transition-all border border-transparent hover:border-primary/20 hover:shadow-sm group"
						>
							...
							<span className="text-sm text-gray-600 font-semibold group-hover:text-tertiary truncate">
								{cat.name}
							</span>
						</label>
					))}
				...
				{errors.categoryIds && (
					<span className="text-xs text-error font-medium">
						{String(errors.categoryIds.message)}
					</span>
				)}
			</section>
		</div>
